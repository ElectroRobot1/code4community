...
export const SLOTS_COLLECTION = "mathLabSlots";
export const BOOKINGS_COLLECTION = "mathLabBookings";
...
/**
 * @param {(slots: object[]) => void} onData
 */
export function subscribeOpenSlots(onData) {
  const db = requireDb();
 ...
 * @param {(slots: object[]) => void} onData
 */
export function subscribeHostSlots(hostId, onData) {
  const db = requireDb();
  const q = query(
    collection(db, SLOTS_COLLECTION),
    where("hostId", "==", hostId),
 ...
 * @param {(bookings: object[]) => void} onData
 */
export function subscribeStudentBookings(studentId, onData) {
  const db = requireDb();
  const q = query(
 ...
 * @param {string} slotId
 */
export async function fetchBookingsForSlot(slotId) {
  const db = requireDb();
  const q = query(
    collection(db, BOOKINGS_COLLECTION),
    where("slotId", "==", slotId),
...
export async function createSlot(params) {
  const db = requireDb();
  const ref = doc(collection(db, SLOTS_COLLECTION));
  ...
  await setDoc(ref, {
    ...
    slotType: params.slotType || "tutoring",
 ...
 * @param {string} hostId
 */
export async function closeSlot(slotId, hostId) {
  const db = requireDb();
 ...
 * @param {string} hostId
 */
export async function deleteSlot(slotId, hostId) {
  const db = requireDb();
 ...
 * @param {string} studentId
 */
export async function cancelBooking(slotId, studentId) {
  const db = requireDb();
  const bookingId = `${slotId}_${studentId}`;
  const slotRef = doc(db, SLOTS_COLLECTION, slotId);
  const bookingRef = doc(db, BOOKINGS_COLLECTION, bookingId);
