  ...
  const allApps = useMemo(
    () => [
      ...
      },
      {
        ...
        isActive: true,
        path: "/yearbook-formatting",
      },
      {
        name: "Seating Chart",
        description: "Classroom seating charts and assignments",
        isActive: true,
        path: "/seating-chart",
      },
      {
        name: "Student Groups",
        description: "Balanced groups with roster and export",
        isActive: true,
        path: "/student-groups",
      },
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredApps.map((app, index) => (
                <button
                  type="button"
                  key={`${app.name}-${index}`}
                  disabled={!app.isActive || !app.path}
                  onClick={() => openApp(app.path)}
                  className={`group card-elevated relative h-[180px] w-full text-left flex flex-col items-center justify-center rounded-xl transition-all duration-300 hover:transform hover:scale-105 hover:shadow-lg disabled:pointer-events-none disabled:opacity-50 ${
                    app.isActive ? "opacity-100" : "opacity-70 hover:opacity-90"
                  }`}
                >
                  <div
